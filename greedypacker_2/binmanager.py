...
class BinManager:
    ...
    def _bin_best_fit(self, item: item.Item) -> bool:
        ...
        item_fits = False
        if (item.total_width <= self.bin_width and
            item.total_height <= self.bin_height):
            item_fits = True
        if (self.rotation and
            (item.total_height <= self.bin_width and
            item.total_width <= self.bin_height)):
            item_fits = True
        if not item_fits:
