...
class MaximalRectangle:
    ...
    def _item_fits_rect(item: Item,
        ...
        if (not rotation and
            item.total_width <= rect.width and
            item.total_height <= rect.height):
            return True
        if (rotation and
            item.total_height  <= rect.width and
            item.total_width <= rect.height):
            return True
        return False
    ...
    def _split_rectangle(rectangle: FreeRectangle,
        ...
        """
        results = []
        if item.total_width < rectangle.width:
            Fw = rectangle.width - item.total_width
            Fh = rectangle.height
            Fx = rectangle.x + item.total_width
            Fy = rectangle.y
            results.append(FreeRectangle(width=Fw,
                                         height=Fh,
                                         x=Fx,
                                         y=Fy))
        if item.total_height < rectangle.height:
            Fw = rectangle.width
            Fh = rectangle.height - item.total_height
            Fx = rectangle.x
            Fy = rectangle.y + item.total_height
            results.append(FreeRectangle(width=Fw,
                                         height=Fh,
                                         x=Fx,
                                         y=Fy))
        #print("Split rectangle",results)
        return results
    ...
    def _item_bounds(item: Item) -> tuple:
        ...
        """
        return (item.x-item.left_clear,
                item.y- item.bottom_clear,
                item.x+item.width+item.right_clear,
                item.y+item.height+item.top_clear)
...
def scoreBAF(rect: FreeRectangle, item: Item, self) -> int:
    """ Best Area Fit """
    return rect.area-item.area, min(rect.width-item.total_width, rect.height-item.total_height)


def scoreBSSF(rect: FreeRectangle, item: Item, self) -> int:
    """ Best Short Side Fit """
    return min(rect.width-item.total_width, rect.height-item.total_height), max(rect.width-item.total_width, rect.height-item.total_height)


def scoreBLSF(rect: FreeRectangle, item: Item, self) -> int:
    """ Best Long Side Fit """
    return max(rect.width-item.total_width, rect.height-item.total_height), min(rect.width-item.total_width, rect.height-item.total_height)


def scoreWAF(rect: FreeRectangle, item: Item, self) -> int:
    """ Worst Area Fit """
    return (0 - (rect.area-item.area)), (0 - min(rect.width-item.total_width, rect.height-item.total_height))


def scoreWSSF(rect: FreeRectangle, item: Item, self) -> int:
    """ Worst Short Side Fit """
    return (0 - min(rect.width-item.total_width, rect.height-item.total_height)), 0 - max(rect.width-item.total_width, rect.height-item.total_height)


def scoreWLSF(rect: FreeRectangle, item: Item, self) -> int:
    """ Worst Long Side Fit """
    return (0 - max(rect.width-item.total_width, rect.height-item.total_height)), (0 - min(rect.width-item.total_width, rect.height-item.total_height))


def scoreBL(rect: FreeRectangle, item: Item, self) -> int:
    """ Bottom Left """
    return rect.y + item.total_height, rect.x
...
def scoreCP(rect: FreeRectangle, item: Item, self) -> int:
    ...
    return (0 - perim), min(rect.width-item.total_width, rect.height-item.total_height)
