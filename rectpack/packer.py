...
class PackerBNFMixin(object):
    ...
    def add_rect(self, width, height, bottom_clear=0, top_clear=0, left_clear=0, right_clear=0, rid = None):
        while True:
...
class PackerBBFMixin(object):
    ...
    def add_rect(self, width, height, bottom_clear=0, top_clear=0, left_clear=0, right_clear=0, rid = None):
        ...
        fit = ((b.fitness(width, height, bottom_clear, top_clear, left_clear, right_clear),  b) for b in self._open_bins)
        fit = (b for b in fit if b[0] is not None)
        try:
            _, best_bin = min(fit, key=self.first_item)
            best_bin.add_rect(width, height, bottom_clear, top_clear, left_clear, right_clear, rid)
            return True
        except ValueError:
            pass
        ...
        while True:
            ...
            # _new_open_bin may return a bin that's too small,
            # so we have to double-check
            if new_bin.add_rect(width, height, bottom_clear, top_clear, left_clear, right_clear, rid):
                return True
...
class Packer(PackerOnline):
    ...
    def add_rect(self, width, height, bottom_clear=0, top_clear=0, left_clear=0, right_clear=0, rid = None):
        self._avail_rect.append((width, height, bottom_clear, top_clear, left_clear, right_clear, rid))
